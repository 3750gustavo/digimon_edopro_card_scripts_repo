...
function s.initial_effect(c)
	Fusion.AddProcMixN(c, true, true,37501122,1,aux.FilterBoolFunction(Card.IsSetCard,0x3763),2)
	...
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET+EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
...
function s.atkup(e,tp,eg,ep,ev,re,r,rp)
	...
	local eu = e:GetHandlerPlayer()
	local tc=c:GetBattleTarget() --assumimos que tc seja o monstro do oponente que estou atacando
	if not tc then return end
	if tc:GetControler() == eu then -- se tc é meu, entao eu que que estou sendo atacado
		tc = tc:GetAttackedGroup():Getfirst() --pegamos quem esta atacando tc, nesse caso o oponente
	end
