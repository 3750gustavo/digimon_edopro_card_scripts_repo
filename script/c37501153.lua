...
function s.initial_effect(c)
	Fusion.AddProcMixN(c, true, true,37501146,1,aux.FilterBoolFunction(Card.IsSetCard,0x3763),1)
	...
	c:RegisterEffect(e4)
	--reduce all opponent monsters atk in 200 points
...
--check if any face-up monsters exist for reducing atk/def
function s.reducetg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
end
--reduce atk/def by 200 on all opponent monsters(mzone)
function s.reduceop(e,tp,eg,ep,ev,re,r,rp)
	...
	--this card c cannot be destroyed by effects anymore this turn
