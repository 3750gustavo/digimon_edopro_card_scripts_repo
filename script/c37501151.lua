...
function s.initial_effect(c)
	-- Fusion Summon
	Fusion.AddProcMixN(c, true, true,37501121,1,aux.FilterBoolFunction(Card.IsSetCard,0x3763),2)
...
function s.filtrofogo(c,tp)
	return c:IsAttribute(ATTRIBUTE_FIRE)
end
function s.valordofogo(e,c)
	return (Duel.GetMatchingGroupCount(s.filtroluz,c:GetControler(),LOCATION_MZONE,LOCATION_MZONE,nil)+Duel.GetMatchingGroupCount(s.filtrofogo,c:GetControler(),LOCATION_MZONE,LOCATION_MZONE,nil))*500
end
function s.GreySword(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	--gain 500atk and def for each LIGHT and FIRE monster ONFIELD
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_SINGLE_RANGE)
	e4:SetCode(EFFECT_UPDATE_ATTACK)
	e4:SetRange(LOCATION_MZONE)
	e4:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END)
	e4:SetValue(s.valordofogo)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	e5:SetCode(EFFECT_UPDATE_DEFENSE)
	c:RegisterEffect(e5)
	--gain aditional attack(can attack twice)
	local e6=e4:Clone()
