...
function s.filtroevolucao(c)
	return (c:IsFaceup() and c:IsSetCard(0xf0f) and c:IsMonster())
end

function s.extra_check(c, mc)
	return c:IsSetCard(0xd74) and c:ListsCodeAsMaterial(mc:GetCode())
end

function s.target(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsMonster() end
	if chk == 0 then return Duel.IsExistingTarget(s.filtroevolucao, tp, LOCATION_ONFIELD, 0, 1, nil) end
	Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TOGRAVE)
	local g = Duel.SelectTarget(tp, s.filtroevolucao, tp, LOCATION_ONFIELD, 0, 1, 1, nil)
